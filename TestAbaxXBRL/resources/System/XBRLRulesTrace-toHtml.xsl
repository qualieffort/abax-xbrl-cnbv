...
<xsl:stylesheet version="2.0" xmlns:exsl="http://exslt.org/common"
    ...
    <xsl:param name="reportTitle">XBRL Rules Report</xsl:param>
    ...
    <xsl:template name="renderHeader" match="formulaTrace">
        ...
        <table width="100%">
            <tr>
                <td align="left"/>
                <td align="right">Copyright  1998 - 2014 R.R. Donnelley &amp; Sons Company. All rights reserved.
                </td>
            </tr>
        </table>
	...
	<!-- This template is responsible for rendering text that might have linebreaks (&#10;) -->
	   ...
	   <xsl:param name="assertion"/>
        <tr valign="top" class="Pass">
            ...
            <td>
            	...
            	<xsl:when test="$assertion/@message">
					<xsl:call-template name="MultilineTextOutput">
						<xsl:with-param name="text" select="$assertion/@message" />
					</xsl:call-template>
	                <!--<xsl:value-of select="$assertion/@message"/>-->
            	</xsl:when>
            	<xsl:otherwise>
