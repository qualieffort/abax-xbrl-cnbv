...
/************ Object: [dbo].[ConsultaRepositorio] ***************/
IF Object_ID(N'[dbo].[ConsultaRepositorio]') is null
BEGIN
CREATE TABLE [dbo].[ConsultaRepositorio](
 [IdConsultaRepositorio] [bigint] IDENTITY(1,1) NOT NULL,
 [Nombre] [varchar](500) NOT NULL,
 ...
 CONSTRAINT [PK_ConsultaRepositorio] PRIMARY KEY CLUSTERED 
(
 [IdConsultaRepositorio] ASC
)WITH (PAD_INDEX  = OFF, STATISTICS_NORECOMPUTE  = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS  = ON, ALLOW_PAGE_LOCKS  = ON) ON [PRIMARY]
) ON [PRIMARY]
SET ANSI_PADDING OFF
ALTER TABLE [dbo].[ConsultaRepositorio]  WITH CHECK ADD  CONSTRAINT [FK_ConsRepo_Usuario] FOREIGN KEY([IdUsuario]) REFERENCES [dbo].[Usuario] ([IdUsuario]) ON DELETE CASCADE
END
GO

/** Agregamos la columna ClaveLista en la tabla de ListaNotificacion si no existe **/
IF NOT EXISTS (SELECT * FROM sys.columns WHERE Name = N'Publica' and Object_ID = Object_ID(N'[dbo].[ConsultaRepositorio]'))
BEGIN
ALTER TABLE [dbo].[ConsultaRepositorio] ADD [Publica] [bit] NOT NULL DEFAULT (1)
ALTER TABLE [dbo].[ConsultaRepositorio]  WITH CHECK ADD  CONSTRAINT [FK_ConsRepo_Usuario] FOREIGN KEY([IdUsuario]) REFERENCES [dbo].[Usuario] ([IdUsuario]) ON DELETE CASCADE
END
GO
...
INSERT [dbo].[Modulo] ([IdModulo], [Nombre], [Descripcion])  (SELECT 15, N'Consultas Especializadas', N'Módulo Administración de las Consultas Especializadas'                   WHERE NOT EXISTS(SELECT IdModulo FROM Modulo WHERE IdModulo = 15))
...
INSERT [dbo].[CategoriaFacultad] ([IdCategoriaFacultad], [Nombre], [Descripcion], [Borrado]) (SELECT 11, N'Facultades Consultas Especializadas', N'Facultades para la administración de las consutas especializadas',    0 WHERE NOT EXISTS (SELECT [IdCategoriaFacultad] FROM [CategoriaFacultad] WHERE [IdCategoriaFacultad] = 11))
...
INSERT [dbo].[Facultad] ([IdFacultad], [Nombre], [Descripcion], [IdCategoriaFacultad], [Borrado]) (SELECT 536, N'Consultar Consultas Repositorio', N'Facultad para las consultas de repositorio.',11,0 WHERE NOT EXISTS(SELECT [IdFacultad] FROM [dbo].[Facultad] WHERE IdFacultad =	536))
INSERT [dbo].[Facultad] ([IdFacultad], [Nombre], [Descripcion], [IdCategoriaFacultad], [Borrado]) (SELECT 537, N'Exportar  Consultas Repositorio', N'Facultad para exportar las consultas de repositorio.',11,0 WHERE NOT EXISTS(SELECT [IdFacultad] FROM [dbo].[Facultad] WHERE IdFacultad =	537))
INSERT [dbo].[Facultad] ([IdFacultad], [Nombre], [Descripcion], [IdCategoriaFacultad], [Borrado]) (SELECT 538, N'Insertar Consulta Repositorio', N'Facultad para agregar una consulta de repositorio.',11,0 WHERE NOT EXISTS(SELECT [IdFacultad] FROM [dbo].[Facultad] WHERE IdFacultad =	538))
INSERT [dbo].[Facultad] ([IdFacultad], [Nombre], [Descripcion], [IdCategoriaFacultad], [Borrado]) (SELECT 539, N'Editar Consulta Repositorio', N'Facultad para editar los datos de una consulta de repositorio.',11,0 WHERE NOT EXISTS(SELECT [IdFacultad] FROM [dbo].[Facultad] WHERE IdFacultad =	539))
INSERT [dbo].[Facultad] ([IdFacultad], [Nombre], [Descripcion], [IdCategoriaFacultad], [Borrado]) (SELECT 540, N'Eliminar Consulta Repositorio', N'Facultad para eliminar una consulta de repositorio.',11,0 WHERE NOT EXISTS(SELECT [IdFacultad] FROM [dbo].[Facultad] WHERE IdFacultad =	540))
...
/** Agregamos la columna AliasClaveCotizacion en Empresa **/
IF NOT EXISTS (SELECT * FROM sys.columns WHERE Name = N'AliasClaveCotizacion' and Object_ID = Object_ID(N'[dbo].[Empresa]'))
BEGIN
ALTER TABLE [dbo].[Empresa] ADD [AliasClaveCotizacion] [varchar] (50) NULL
END
GO