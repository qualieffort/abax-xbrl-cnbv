...
DECLARE
  ...
  existeTipoConsulta number(1);
  existeIdTaxonomiaAnalisis number(1);
  existeAnioDocumentoInsntacia number(1);
  existeTrimestreDocumentoInst number(1);
BEGIN
...
-- Columna TipoConsulta
  SELECT CASE WHEN EXISTS (SELECT * FROM ALL_TAB_COLUMNS WHERE COLUMN_NAME = 'TipoConsulta' AND  TABLE_NAME = 'ConsultaAnalisis' AND OWNER = 'ABAX_XBRL') then 1 else 0 end  into existeTipoConsulta FROM DUAL;
  IF existeTipoConsulta = 0
  THEN
      DBMS_OUTPUT.put_line('Se agrega columna TipoConsulta');
      EXECUTE IMMEDIATE 'ALTER TABLE "ABAX_XBRL"."ConsultaAnalisis" ADD ("TipoConsulta" NUMBER(10) NULL)';
  ELSE
     DBMS_OUTPUT.put_line('Ya existe la columna TipoConsulta');
  END IF;

-- Columna IdTaxonomiaXbrl
  SELECT CASE WHEN EXISTS (SELECT * FROM ALL_TAB_COLUMNS WHERE COLUMN_NAME = 'IdTaxonomiaXbrl' AND  TABLE_NAME = 'ConsultaAnalisis' AND OWNER = 'ABAX_XBRL') then 1 else 0 end  into existeIdTaxonomiaAnalisis FROM DUAL;
  IF existeIdTaxonomiaAnalisis = 0
  THEN
      DBMS_OUTPUT.put_line('Se agrega columna IdTaxonomiaXbrl');
      EXECUTE IMMEDIATE 'ALTER TABLE "ABAX_XBRL"."ConsultaAnalisis" ADD ("IdTaxonomiaXbrl" NUMBER(19) NULL)';
  ELSE
     DBMS_OUTPUT.put_line('Ya existe la columna IdTaxonomiaXbrl');
  END IF;
  
-- Columna Anio
  SELECT CASE WHEN EXISTS (SELECT * FROM ALL_TAB_COLUMNS WHERE COLUMN_NAME = 'Anio' AND  TABLE_NAME = 'DocumentoInstancia' AND OWNER = 'ABAX_XBRL') then 1 else 0 end  into existeAnioDocumentoInsntacia FROM DUAL;
  IF existeAnioDocumentoInsntacia = 0
  THEN
      DBMS_OUTPUT.put_line('Se agrega columna Anio');
      EXECUTE IMMEDIATE 'ALTER TABLE "ABAX_XBRL"."DocumentoInstancia" ADD ("Anio" NUMBER(10))';
  ELSE
     DBMS_OUTPUT.put_line('Ya existe la columna Anio');
  END IF;
  
-- Columna Trimestre
  SELECT CASE WHEN EXISTS (SELECT * FROM ALL_TAB_COLUMNS WHERE COLUMN_NAME = 'Trimestre' AND  TABLE_NAME = 'DocumentoInstancia' AND OWNER = 'ABAX_XBRL') then 1 else 0 end  into existeTrimestreDocumentoInst FROM DUAL;
  IF existeTrimestreDocumentoInst = 0
  THEN
      DBMS_OUTPUT.put_line('Se agrega columna Trimestre');
      EXECUTE IMMEDIATE 'ALTER TABLE "ABAX_XBRL"."DocumentoInstancia" ADD ("Trimestre" NUMBER(10))';
  ELSE
     DBMS_OUTPUT.put_line('Ya existe la columna Trimestre');
  END IF;
