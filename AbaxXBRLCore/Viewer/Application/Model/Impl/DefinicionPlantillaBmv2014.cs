...
{
    /// <summary>
    /// Implementación específica de la definición de la plantilla para la taxonomía de la BMV 2014.
    /// <author>José Antonio Huizar Moreno</author>
    /// <version>1.0</version>
    /// </summary>
    public class DefinicionPlantillaBmv2014 : DefinicionPlantillaXbrlAbstract
    {
        ...
        public override string ObtenerTituloEspecificoDocumentoXbrl(DocumentoInstanciaXbrlDto instancia = null) { 
            if(this.parametrosConfiguracion != null && this.parametrosConfiguracion.ContainsKey("trimestre") && 
                this.parametrosConfiguracion.ContainsKey("emisora") && this.parametrosConfiguracion.ContainsKey("anio")){
                var trimestre = parametrosConfiguracion["trimestre"];
                var emisora = this.parametrosConfiguracion["emisora"];
                var nAnio = 0;
                DateTime fechaAnio = new DateTime();
                if (XmlUtil.ParsearUnionDateTime(this.parametrosConfiguracion["anio"], out fechaAnio, DateTimeStyles.AdjustToUniversal))
                {
                    nAnio = fechaAnio.Year;
                }
                if(trimestre.Equals("4D")){
                    trimestre = "4";
                }
                return "ifrsxbrl_" + emisora + "_" + nAnio + "-" + trimestre;
            }
            return null;            
        }
        ...
        public override IDictionary<string, string> DeterminaParametrosConfiguracionDocumento(DocumentoInstanciaXbrlDto instancia)
        {
            ...
            if (instancia.HechosPorIdConcepto.ContainsKey("ifrs_mx-cor_20141205_NumeroDeTrimestre"))
            {
                var hechosNumeroTrimestre = instancia.HechosPorIdConcepto["ifrs_mx-cor_20141205_NumeroDeTrimestre"];
