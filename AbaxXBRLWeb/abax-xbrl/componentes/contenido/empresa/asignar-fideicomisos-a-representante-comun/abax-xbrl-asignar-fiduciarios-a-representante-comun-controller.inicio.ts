module abaxXBRL.componentes.controllers {
    ...
    **/
    export interface AbaxXBRLAsignaFiduciariosRepresentanteComunScope extends IAbaxXBRLInicioScope {
        /**
        * Lista de empresas primarias.
        **/
        empresasPrimarias: Array<abaxXBRL.shared.modelos.IEmisora>;
        /**
        * Empresa primaria seleccionada.
        **/
        empresaPrimariaSeleccionada: abaxXBRL.shared.modelos.IEmisora;
        /**
        * Bandera que indica que se esta esperando la respuesta del servdor a la petición de empresas primarias.
        **/
        cargandoEmpresasPrimarias: boolean;
        /**
        ...
        **/
        listaExistentes: Array<shared.modelos.IEmisora>
        /**
        ...
        **/
        listaAsignados: Array<shared.modelos.IEmisora>
        /**
        * Bandera que indica que se están cargando las empresas del elemento actual.
        **/
        cargandoAsignados: boolean;
        /**
        * Diccionario con checks de las propieadades seleccinadas.
        **/
        listaChecks: { [idEmpresa: number]: boolean }
        /**
        * Asinga las empresas secimdarias seleccionadas a la empresa primaria actual.
        **/
        asignarTodos();
        /**
        * Quita las empresas secundarias seleccionadas de la empresa primaria actual.
        **/
        quitarTodos();
        /**
        * Arreglo con los asignables que no han sido asignados al elemento actual.
        **/
        listaNoAsignados: Array<shared.modelos.IEmisora>
        /**
        * Acutaliza las empresas asignadas de la empresa primaria actual.
        **/
        guardar();
        /**
        * Bandera con el valor del checkbox que cambia el estado de checado de las empresas de la izquierda.
        **/
        checkAllLeft: boolean;
        /**
        * Bandera con el valor del checkbox que cambia el estado de checado de las empresas de la derecha.
        **/
        ...
        **/
        guardando: boolean;
        /**
        * Listener a ejecutar cuando cambia el valor seleccionado del filtro de empresas primarias.
        **/
        onChangeEmpresa();
        /**
    ...
    **/
    export class AbaxXBRLAsignaFiduciariosRepresentanteComunController {
        /** 
       * El scope del controlador 
       **/
        private $scope: AbaxXBRLAsignaFiduciariosRepresentanteComunScope;
        ...
        **/
        private determinaNoAsignados(): void {
            ...
            var listaNoAsignados = new Array<shared.modelos.IEmisora>();
            var listCheck: { [idEmpresa: number]: boolean } = {};
            ...
            for (var indexExistente = 0; indexExistente < listaExistentes.length; indexExistente++) {
                ...
                var esRepresentanteComun = listaExistentes[indexExistente].RepresentanteComun === true ? true : false;
                
                for (var indexAsignados = 0; indexAsignados < listaAsignados.length; indexAsignados++) {
                    var asignado = listaAsignados[indexAsignados];
                    if (existente.IdEmpresa == asignado.IdEmpresa) {
                        estaAsignado = true;
                        break;
                    }
                }

                if (!estaAsignado && !esRepresentanteComun) {
                    listaNoAsignados.push(existente);
                }
                listCheck[existente.IdEmpresa] = false;
            }
        ...
        /**
        * Obtiene los elementos asignados a la empresa primaria seleccionada.
        **/
        private obtenAsignados(): void {
            ...
            if (!scope.empresaPrimariaSeleccionada) {
                return;
            }
            var url = AbaxXBRLConstantes.OBTEN_FIDUCIARIOS_ASIGNADOS_REP_COMUN_PATH;
            var onSucess = function (result: any) { self.determinaNoAsignados(); };
            var idEmpresa = scope.empresaPrimariaSeleccionada.IdEmpresa;
        ...
        /**
        * Obtiene las empresas secundarias existentes.
        **/
        private obtenExistentes(): void {
            var self = this;
            var scope = self.$scope;
            var url = AbaxXBRLConstantes.CONSULTA_FIDUCIARIOS_DISPONIBLES_REP_COMUN_PATH;
            var onSucess = function (result: any) { self.obtenAsignados(); };
            var idEmpresa = scope.empresaPrimariaSeleccionada.IdEmpresa;
            var parametros: { [id: string]: string } = { "idFideicomitente": idEmpresa.toString() };
            this.abaxXBRLRequestService.asignaResultadoPost(url, parametros, scope, 'listaExistentes', 'cargandoExistentes').then(onSucess);
        }

        /**
       * Asigna las empresas secundarias asignadas a la empresa primaria.
       **/
        private guardar() {
            var self = this;
            var scope = self.$scope;
            ...
            var url = AbaxXBRLConstantes.ASIGNA_FIDUCIARIOS_A_REPRESENTATE_COMUN_PATH;
            var idsAsignados = new Array<number>();
            for (var index = 0; index < listaAsignados.length; index++) { idsAsignados.push(listaAsignados[index].IdEmpresa); }
        ...
        **/
        private onGuardarSucess(resultado: shared.modelos.IResultadoOperacion) {
            ...
            if (resultado.Resultado) {
                var mensaje = util.getValorEtiqueta("MENSAJE_EXITO_ACTUALIZAR_FIDEICOMITENTES", {
                    "NOMBRE": self.$scope.empresaPrimariaSeleccionada.NombreCorto
                });
                util.ExitoBootstrap(mensaje);
                util.cambiarEstadoVistasA("inicio.empresa.indice");
            } else {
                var mensaje = util.getValorEtiqueta("MENSAJE_ERROR_ACTUALIZAR_FIDEICOMITENTES", {
                    "NOMBRE": self.$scope.empresaPrimariaSeleccionada.NombreCorto
                });
       ...
       **/
        private getElementosChecados(evaluar: Array<shared.modelos.IEmisora>): Array<shared.modelos.IEmisora> {
            var checks = this.$scope.listaChecks;
            var checadas = new Array<shared.modelos.IEmisora>();
            for (var index = 0; index < evaluar.length; index++) {
                var evaluado = evaluar[index];
                if (checks[evaluado.IdEmpresa]) {
        ...
        **/
        private setCheckAll(evaluar: Array<shared.modelos.IEmisora>, value: boolean) {
            if (evaluar && evaluar.length > 0) {
                var checks = this.$scope.listaChecks;
                for (var index = 0; index < evaluar.length; index++) {
                    var evaluado = evaluar[index];
                    checks[evaluado.IdEmpresa] = value;
                }
            }
        }
        /**
        ...
        **/
        ...
        /**
        * Llena el listado de empresas primarias.
        **/
        private cargarEmisoras(empresaYaSeleccionado = false): void {
            var self: AbaxXBRLAsignaFiduciariosRepresentanteComunController = this;
            var scope: AbaxXBRLAsignaFiduciariosRepresentanteComunScope = self.$scope;

            var onSuccess = function () {
                self.obtenExistentes();
            }
            var parametros: { [id: string]: string } = {};
            var url = AbaxXBRLConstantes.CONSULTA_TODAS_EMPRESAS_COMBO_PATH;
            self.abaxXBRLRequestService.asignaResultadoPost(url, parametros, scope, 'empresasPrimarias', 'cargandoEmpresasPrimarias').then(onSuccess);
        }
        ...
        **/
        private init() {
            ...
            scope.empresasPrimarias = Array<shared.modelos.IEmisora>();
            scope.empresaPrimariaSeleccionada = { IdEmpresa: null, NombreCorto: "" };            
                        
            var empresaPrimariaSeleccionadaAux = util.getAtributoTemporal(AbaxXBRLConstantes.ATRIBUTO_SESION_ID_ELEMENTO_PROCESADO);
            scope.empresaPrimariaSeleccionada = empresaPrimariaSeleccionadaAux;
            scope.empresaPrimariaSeleccionada.IdEmpresa = parseInt(empresaPrimariaSeleccionadaAux.IdEmpresa);
            
            scope.cargandoEmpresasPrimarias = true;
            scope.filtroLeft = '';
            scope.filtroRight = '';
            scope.listaExistentes = Array<shared.modelos.IEmisora>();
            scope.cargandoExistentes = true;
            scope.listaAsignados = Array<shared.modelos.IEmisora>();
            ...
            scope.listaNoAsignados = Array<shared.modelos.IEmisora>();
            scope.checkAllLeft = false;
            scope.checkAllRight = false;
            scope.guardando = false;
            scope.onChangeEmpresa = function () { self.onChangeEmpresa(); };
        ...
        **/
        constructor($scope: AbaxXBRLAsignaFiduciariosRepresentanteComunScope, abaxXBRLRequestService: shared.service.AbaxXBRLRequestService, $state: ng.ui.IStateService) {
            this.$scope = $scope;
    ...
    AbaxXBRLAsignaFiduciariosRepresentanteComunController.$inject = ['$scope', 'abaxXBRLRequestService', '$state'];
} 