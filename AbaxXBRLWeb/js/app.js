...
+function ($) {
  ...
  $(function(){
    ...
    $(document).ready(function () {
        buscarUltimasAlertas();
        if (intervalo == '') {
            intervalo = setTimeout(buscarUltimasAlertas, 30000);
        }
    });
  });
}(jQuery);
...
function buscarUltimasAlertas() {
    /*
    $.post('/Home/ObtenerUltimasAlertasUsuario',
        {
            numeroAlertas: 5
            ...
            if (data) {
                $('#listadoAlertas').html('');
                if (data.length == 0) {
                    $('#tituloAlertas').html('<strong>No ha recibido ninguna alerta aún.</strong>');
                    $('#listadoAlertas').html('<a href="#" class="media list-group-item"> ' +
                                              '    <span class="media-body block m-b-none">No hay alertas para el usuario.<br> ' +
                                              '    </span> ' +
                                              '</a>');
                } else {
                    $('#tituloAlertas').html('<strong>&Uacute;ltimas <span id="numeroAlertas" class="count">' + data.length + '</span> alertas recibidas.</strong>');
                    for (i = 0; i < data.length; i++) {
                        $('#listadoAlertas').append('<a href="#" class="media list-group-item"> ' +
                                                    '    <span class="pull-left thumb-sm avatar m-r"> ' +
                                                    '       <img src="/images/a0.png" alt="..."> ' +
                                                    '       <i class="' + (data[i].EsCorrecto ? 'on' : 'busy') +  ' b-white bottom"></i> ' +
                                                    '    </span> ' +
                                                    '    <span class="clear">' +
                                                    '       <span>' + data[i].NombreUsuario + '</span>' +
                                                    '       <small class="text-bold clear">' + data[i].Fecha + '</small>' +
                                                    '       <small class="text-muted clear text-ellipsis">' + data[i].Contenido + '</small>' +
                                                    '    </span>' +
                                                    //'    <span class="media-body block m-b-none">' + data[i].NombreUsuario + ' - ' + data[i].Fecha + ' - ' + data[i].Contenido + '</small> ' +
                                                    //'    </span> ' +
                                                    '</a>');
                    }
                }
